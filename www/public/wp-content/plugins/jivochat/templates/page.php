...
<div class="wrap">
    ...
    <?php if(!$this->widget_id){ ?>
        ...
        <p class="gray" style="margin-top: 0px;"><?php echo str_replace('%JIVO_URL%',JIVO_URL,__('If you need help, please chat with us on <a href="%JIVO_URL%">our website</a>','jivosite')); ?></p>
    ...
    ?>
        <div class="success">
            <?php _e('Congratulations! You have successfully installed JivoChat on your website. Now you need to install the agent’s app on your computer and customize the chat window in the Jivo App.','jivosite'); ?>
        </div>
        <div class="gray_form">
            <h3>1. <?php _e('Install Jivo App','jivosite'); ?></h3>
            ...
            <h3>2. <?php _e('Customize Settings and add Agents in the App','jivosite'); ?></h3>
            <p><?php _e('Please login to the app to add more agents’ accounts, customize the chat window settings and set up proactive invitations to get the most from your new live chat!','jivosite'); ?></p>
            <a  class="button button-primary"  href='<?php if($this->token){ echo JIVO_INTEGRATION_URL.'/login?token='.$this->token.(JIVO_LANG!='ru'?"&lang=".JIVO_LANG:''); }else{echo 'https://app.jivosite.com';} ?>' target="_blank"><?php _e('Open web version of the app','jivosite'); ?></a>
            <p><a href="?<?php echo http_build_query($_GET) ?>&mode=reset"><?php _e('Reset account info','jivosite'); ?></a></p>
        </div>
        <p class="gray"><?php echo str_replace('%JIVO_URL%',JIVO_URL,__('If you need help, please chat with us on <a href="%JIVO_URL%">our website</a>','jivosite')); ?></p>
    <?php } ?>
</div>